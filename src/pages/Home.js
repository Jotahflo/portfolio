...
const Home = () => {
  return (
    <div>
      ...
      <ScrollDownButton iconButtonLeft={iconMouse} iconButtonRight={iconArrowDown} nameIdScroll="sectionProjects" classButton="text-white p-4 flex flex-row" />
      <div id="sectionHome" className="bg-gray-800">
        <div className="container mx-auto pt-4 pb-8 grid grid-cols-1 md:grid-cols-2 items-center justify-items-center h-full md:h-screen w-screen">
          <div className="md:order-2 pb-4 lg:w-4/5">
            <img src={logoJF} alt='Logo Juan Flores'/>
          </div>
          <div className="md:order-1">
            <p className="text-4xl text-white font-serif pb-s3">Hello, I'm Juan</p>
            <p className="text-xl text-gray-200 font-medium pb-3">Frontend Developer</p>
            <p className="text-base text-gray-400 pb-4">I have great knowledge in the Area of Informatica and Years of Experience, working with Web and Mobile technologies, both in Frontend, and Backend.</p>
            <ActionButton titleButton="What can I offer?" iconButton={iconUser} styleDivMaster="md:flex" />
          </div>
        </div>
      </div>
